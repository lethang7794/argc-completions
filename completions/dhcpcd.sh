...
# @option -c --script <script>                  Use this script instead of the default /usr/libexec/dhcpcd-run-hooks.
...
# @option -o --option <option>                  Request the DHCP option variable for use in /usr/libexec/dhcpcd-run-hooks.
...
# @flag -T --test                               On receipt of DHCP messages just call /usr/libexec/dhcpcd-run-hooks with the reason of TEST which echos the DHCP variables found in the message to the console.
