...
# @option -g --group[`_module_os_group`] <groupname>  Change to this group after startup (defaults to dnsmasq).
...
# @option -l --dhcp-leasefile <path>               Specify where to store DHCP leases (defaults to /var/lib/dnsmasq/dnsmasq.leases).
...
# @option -x --pid-file <path>                     Specify path of PID file (defaults to /run/dnsmasq.pid).
